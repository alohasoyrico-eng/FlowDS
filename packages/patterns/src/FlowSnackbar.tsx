...
import { FlowIcon, GrowthObserver, type Tone, useFlowDefaultSize } from "@flow/primitives";
import { FlowButton, FlowIconButton } from "@flow/components";
...
export interface SnackbarItem {
  ...
  message: string;
  /** Visual status indicating severity */
  ...
  function FlowSnackbarProvider({ children, ...rest }, ref) {
    ...
    const show = useCallback(
      ({
        ...
        setItems((prev) => [...prev, { id, message, status, action, duration, size, tone }]);
      },
      [],
    );
    ...
    return (
      <SnackbarContext.Provider value={{ show }}>
        ...
        {/* Toast container */}
        <div
          className="flow-snackbar-container"
          aria-live="polite"
...
const snackbarIconMap: Record<SnackbarStatus, string> = {
  info: "info",
  success: "check",
  warning: "warning",
  error: "warning",
...
const FlowSnackbarItem = forwardRef<
  HTMLDivElement,
  {
    item: SnackbarItem;
...
>(function FlowSnackbarItem({ item, onDismiss, ...rest }, ref) {
  ...
  return (
    <GrowthObserver event="flow.snackbar.impression" properties={{ status: item.status }} inline>
      <div
        ref={ref}
        ...
        {...rest}
      >
        <FlowIcon name={snackbarIconMap[item.status]} size="sm" />
