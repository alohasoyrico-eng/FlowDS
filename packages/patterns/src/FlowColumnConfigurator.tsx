...
import { GrowthObserver, Inline, Text } from "@flow/primitives";
import { FlowCheckbox, FlowIconButton } from "@flow/components";
...
export const FlowColumnConfigurator = forwardRef<HTMLDivElement, ColumnConfiguratorProps>(
  function FlowColumnConfigurator(
    { columns, onColumnsChange, className = "", style, ...rest },
    ref,
  ) {
    ...
    return (
      <GrowthObserver event="flow.column-configurator.impression">
        <div
          ref={setRootRef}
          {...rest}
          ...
          {open && (
            ...
            >
              {columns.map((col, i) => (
                ...
                >
                  <FlowCheckbox checked={col.visible} onChange={() => toggle(col.key)} size="sm" />
                  <Text
                    variant="caption"
                    color={col.visible ? "primary" : "tertiary"}
                    style={{ flex: 1 }}
                  >
                    {col.label}
                  </Text>
