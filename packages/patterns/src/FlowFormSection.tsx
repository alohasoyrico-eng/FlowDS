...
import { ActionSurface, FlowIcon, GrowthObserver, Stack, Surface, Text } from "@flow/primitives";
...
export const FlowFormSection = forwardRef<HTMLDivElement, FormSectionProps>(
  function FlowFormSection(
    ...
    return (
      <GrowthObserver event="flow.form-section.impression">
        <Surface
          ref={ref}
          {...rest}
          border={false}
          padding={0}
          elevation={0}
          className={`flow-form-section ${className}`}
          style={style}
        >
          {collapsible ? (
            <ActionSurface
