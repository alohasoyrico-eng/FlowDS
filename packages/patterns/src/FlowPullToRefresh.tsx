...
import { GrowthObserver, Stack, Text } from "@flow/primitives";
...
export const FlowPullToRefresh = forwardRef<HTMLDivElement, PullToRefreshProps>(
  function FlowPullToRefresh(
    ...
    return (
      <GrowthObserver event="flow.pull-to-refresh.impression">
        <div
          ref={setRootRef}
          {...rest}
            ...
            <Stack gap={1} align="center">
              <FlowCircularProgress
                ...
                value={
                  phase === "refreshing"
                    ? undefined
                    : Math.min(100, (pullDistance / threshold) * 100)
                }
                aria-label={phase === "refreshing" ? "Refreshing" : "Pull progress"}
                style={{ opacity: pullDistance / threshold }}
              />
              <Text variant="caption" className="flow-pull-to-refresh-text">
                {indicatorText}
              </Text>
            </Stack>
          </div>
