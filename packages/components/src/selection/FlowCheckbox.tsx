...
export interface CheckboxProps {
  ...
  "aria-label"?: string;
  /** Tab index override — use -1 to make checkbox non-focusable inside composite widgets */
  tabIndex?: number;
}

export const FlowCheckbox = forwardRef<HTMLLabelElement, CheckboxProps>(function FlowCheckbox(
  {
    checked,
    defaultChecked,
    indeterminate,
  ...
  if (label) {
    return (
      <div className="flow-checkbox-wrapper" data-size={resolvedSize}>
        {inner}
        <Text as="label" variant="label-m" htmlFor={inputId} className="flow-checkbox-label-text">
          {label}
        </Text>
      </div>
    );
