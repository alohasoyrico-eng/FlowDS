...
import "../../css/navigation/BottomNav.css";
...
export const FlowBottomNav = forwardRef<HTMLElement, BottomNavProps>(function FlowBottomNav(
  {
    items = [],
    activeKey,
    onChange,
  ...
  return (
    <GrowthObserver event="flow.bottom-nav.impression" properties={{}} inline>
      <nav
        ref={ref}
        {...rest}
        className={`flow-bottom-nav ${className}`}
        ...
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              {showLabels && (
                <Text as="span" variant="caption" className="flow-bottom-nav-label">
                  {item.label}
                </Text>
              )}
            </button>
