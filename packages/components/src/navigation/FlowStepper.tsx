...
export const FlowStepper = forwardRef<HTMLElement, StepperProps>(function FlowStepper(
  {
    steps,
    activeStep,
    onStepClick,
  ...
  return (
    <GrowthObserver
      event="flow.stepper.impression"
      properties={{ orientation, size: resolvedSize }}
      inline
    >
      <nav
        ref={ref}
      ...
      >
        <ol className="flow-stepper-list">
          {steps.map((step, index) => {
            ...
            const isClickable =
              clickable && status === "complete" && onStepClick && !disabled && !loading;
            const isLast = index === steps.length - 1;

            return (
              <li
                key={step.id ?? step.key ?? `step-${index}`}
                ...
                <div className="flow-stepper-content">
                  ...
                  {step.description && (
                    <Text
                      ...
                      variant="paragraph-s"
                      color="secondary"
                      className="flow-stepper-description"
                    >
                      {step.description}
                    </Text>
                  )}
