...
import { Stack, stateAttrs, Text } from "@flow/primitives";
...
export const FlowFieldset = forwardRef<HTMLFieldSetElement, FieldsetProps>(function FlowFieldset(
  { legend, description, children, disabled, error, className = "", style, ...rest },
  ref,
) {
  return (
    <fieldset
    ...
    >
      ...
      {description && (
        <Text
          variant="caption"
          color="secondary"
          style={{ marginBottom: "var(--ref-frame-space-3)" }}
        >
          {description}
        </Text>
      )}
