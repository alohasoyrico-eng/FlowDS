...
import "../../css/navigation/Accordion.css";
import {
  type CSSProperties,
  forwardRef,
  type ReactNode,
...
import { FlowIcon, stateAttrs, Text, useFlowDefaultSize } from "@flow/primitives";

type AccordionSize = "sm" | "md" | "lg" | "xl";

/** Resolve canonical id from AccordionItem (id > key) */
function resolveItemId(item: AccordionItem): string {
...
export const FlowAccordion = forwardRef<HTMLDivElement, AccordionProps>(function FlowAccordion(
  {
    items: itemsProp,
    multiple = false,
  ...
  const items: AccordionItem[] =
    itemsProp ??
    (title !== undefined ? [{ id: "__single__", label: title, content: children, disabled }] : []);
  ...
  return (
    ...
    >
      {items.map((item) => {
        ...
        const triggerId = `${idPrefix}-trigger-${itemId}`;
        const panelId = `${idPrefix}-panel-${itemId}`;
              ...
              <span className="flow-accordion-trigger-content">
                ...
                {item.subtitle && (
                  <Text
                    ...
                    variant="paragraph-s"
                    color="secondary"
                    className="flow-accordion-trigger-subtitle"
                  >
                    {item.subtitle}
                  </Text>
                )}
