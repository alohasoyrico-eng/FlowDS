...
describe("FlowKPICard — rendering", () => {
  ...
  it("renders the description when provided", () => {
    render(
      <FlowKPICard value="99%" label="Uptime" description="Last 30 days" />,
    );
  ...
  it("renders the trend value when provided", () => {
    render(
      <FlowKPICard value="42" label="Issues" trend="up" trendValue="+12.5%" />,
    );
  ...
  it("renders a loading skeleton when loading is true", () => {
    const { container } = render(
      <FlowKPICard value="0" label="Loading" loading />,
    );
  ...
  it("sets aria-invalid when error is true", () => {
    const { container } = render(
      <FlowKPICard value="N/A" label="Error" error />,
    );
...
describe("FlowKPICard — interaction", () => {
  it("calls onClick when the card is clicked", async () => {
    const onClick = vi.fn();
    render(
      <FlowKPICard value="1,234" label="Total Users" onClick={onClick} />,
    );
  ...
  it("applies cursor:pointer when onClick is provided", () => {
    const { container } = render(
      <FlowKPICard value="42" label="Clickable" onClick={vi.fn()} />,
    );
