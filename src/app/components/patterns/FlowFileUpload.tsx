...
import { FlowProgressBar } from "../feedback/FlowProgressBar";
...
export const FlowFileUpload = forwardRef<HTMLDivElement, FileUploadProps>(
  function FlowFileUpload({
  accept,
  multiple = true,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <FlowButton variant="low" size="sm" leadingIcon="upload">
          Browse
        </FlowButton>
      </div>
        ...
        <div className="flow-file-upload-list">
          {files.map((f) => (
            <div key={f.id} className="flow-file-upload-item" data-status={f.status}>
              <FlowIcon
                name={
                  f.status === "error" ? "alert-circle" : f.status === "success" ? "check" : "file"
                }
                size="sm"
                color={
                  f.status === "error"
