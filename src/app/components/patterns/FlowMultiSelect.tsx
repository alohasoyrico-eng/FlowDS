...
import { FlowIcon, GrowthObserver, Inline, Stack, stateAttrs, Text } from "../../primitives";
...
export const FlowMultiSelect = forwardRef<HTMLDivElement, MultiSelectProps>(
  ...
  return (
    <GrowthObserver event="flow.multi-select.impression" properties={{ size: resolvedSize }} inline>
    <div
      ref={setRootRef}
    ...
    >
      ...
      >
        ...
        {selectedOptions.length > maxDisplayChips && (
          <FlowChip size="sm" variant="neutral">
            +{selectedOptions.length - maxDisplayChips} more
          </FlowChip>
        )}

        {/* Placeholder — stays visible when open (search is in dropdown) */}
        {!hasValue && <Text role="label-m" className="flow-multi-select-placeholder">{placeholder}</Text>}
        ...
        {name && currentValues.map((v) => <input key={v} type="hidden" name={name} value={v} />)}
      </div>
            ...
            {filteredOptions.length === 0 ? (
              ...
              Array.from(groupedOptions.entries()).map(([group, opts]) => (
                <React.Fragment key={group || "__ungrouped"}>
                  {group && (
                    <div role="group" aria-label={group} className="flow-multi-select-group-label">
                      {group}
                    </div>
                  )}
                  {opts.map((option) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span role="presentation" onClick={(e) => e.stopPropagation()}>
                          <FlowCheckbox checked={isSelected} onChange={() => toggleOption(option)} size="sm" />
                        </span>
