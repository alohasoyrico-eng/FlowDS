  ...
  return (
    ...
    >
      ...
      >
        <Stack gap="component">
          <Stack gap={2}>
            <Text role="heading-m" id={titleId}>
              {title}
            </Text>
            {typeof message === "string" ? <Text role="paragraph-s">{message}</Text> : message}
          </Stack>
          <Inline gap={2} justify="end">
            <FlowButton variant="medium" size="sm" onClick={onCancel}>
              {cancelLabel}
            </FlowButton>
            <FlowButton
              variant={variant === "danger" ? "danger" : "high"}
              size="sm"
              onClick={onConfirm}
              disabled={loading}
            >
