...
import React, { forwardRef, type KeyboardEvent, useEffect, useId, useMemo, useRef, useState } from "react";

import { useFlowFocusTrap } from "../../hooks/use-flow-focus-trap";
import { FlowIcon, Inline, Stack, Text } from "../../primitives";
...
export const FlowCommandPalette = forwardRef<HTMLDivElement, CommandPaletteProps>(
  ...
  useEffect(() => {
    ...
    const handler = (e: Event) =>
      handleKeyDownRef.current(e as unknown as KeyboardEvent);
    ...
    >
      ...
      >
        ...
        <div
          ...
          id={listId}
          role="listbox"
          style={{ maxHeight: "var(--comp-command-palette-max-height)", overflowY: "auto", padding: "var(--ref-frame-space-1)" }}
        >
          {filtered.length === 0 ? (
                ...
                {items.map((cmd) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text role="paragraph-s" style={{ flex: 1 }} color="inherit">
                        {cmd.label}
                      </Text>
                      {cmd.shortcut && (
                        <kbd
