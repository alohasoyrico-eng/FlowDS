...
import { FlowIcon, GrowthObserver, stateAttrs, Text } from "../../primitives";
...
export const FlowAvatar = forwardRef<HTMLSpanElement, AvatarProps>(
  function FlowAvatar({
    src,
    alt = "",
    initials,
  ...
  return (
    <GrowthObserver event="flow.avatar.impression" properties={{ size: resolvedSize, shape }} inline>
    <span
      ref={ref}
      {...rest}
    ...
    >
      {showImage && (
        <img className="flow-avatar-image" src={src} alt={alt} onError={() => setImgError(true)} />
      )}
      {showInitials && <Text as="span" role="caption" color="inherit" className="flow-avatar-initials">{resolvedInitials}</Text>}
