...
import { GrowthObserver, Overlay, Surface, Text, type Tone } from "../../primitives";
...
export const FlowDialog = forwardRef<HTMLDivElement, DialogProps>(
  function FlowDialog(
    {
      open,
      onClose,
          ...
          >
            ...
            {(title || description) && (
              <div className="flow-dialog-header">
                {title && <Text as="h2" role="heading-m" id={titleId} className="flow-dialog-title">{title}</Text>}
                {description && <Text as="p" role="paragraph-s" color="secondary" id={descId} className="flow-dialog-description">{description}</Text>}
              </div>
            )}
